...
function Collection:count()
    local i = 0;
    for _, _ in pairs(self.table) do
        i = i + 1;
    end
    return i;
end
...
intersection = collect(intersection):flip():all()
...
function Collection:shift()
	-- This loop is executed at once but we use it to get the key easily.
    for key, value in pairs(self.table) do
        if type(key) == 'number' then
            return table.remove(self.table, key);
		elseif(key == nil) then -- This line as only for purpose to remove a linter warning lol
        else
            self.table[key] = nil;
            return value;
        end
...
function Collection:shuffle()
    ...
    for key, value in pairs(self.table) do
        if type(key) == 'number' then
            shuffled:set(
				table.remove(numericKeys,
					math.random(#numericKeys)),
					table.remove(numericValues,
					math.random(#numericValues)
				)
			);
