...
ISScrollBarMinimalist = ISUIElement:derive("ISScrollBarMinimalist");
...
function ISScrollBarMinimalist:instantiate()
	...
	if self.vertical then
		...
		self.anchorBottom = true
		self.x = 0
		self.y = self.parent.height - 5 -- FIXME, height is 17
...
--				self:repaintStencilRect(0, 0, self.width, self.height)
		...
		if sw > self:getWidth() then
			...
			self:drawTextureScaled(self.midtex, self.barx, 0, self.barwidth, 2, 1, 1, 1, 1)
